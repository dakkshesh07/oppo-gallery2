...
.method public createQuery()Ljava/lang/String;
    ...
    const-string p0, "UPDATE OR ABORT `resource_song` SET `song_id` = ?,`position` = ?,`tag` = ?,`source` = ?,`duration` = ?,`zh_name` = ?,`ch_name` = ?,`en_name` = ?,`thumbnail_url` = ?,`thumbnail_path` = ?,`resource_url` = ?,`resource_path` = ?,`resource_hash` = ?,`resource_size` = ?,`category` = ?,`download_state` = ?,`builtin` = ?,`new_resource` = ?,`version` = ?,`update_time` = ? WHERE `song_id` = ?"
